...
public class UpdateContext : ActionNode
{
    ...
    private Dictionary<string, ActionAttack> CalcHittingAttacks()
    {
        ...
        foreach (KeyValuePair<string, ActionAttack> attack in context.selfFSM.AttackMoveDict)
        {
            ...
            if (context.distanceToOpponent.x < (attack.Value.HitboxSize.x + attack.Value.HitboxOffset.x + context.attackBoxFlexibilityMargin) &&
                context.distanceToOpponent.x > (attack.Value.HitboxSize.x - attack.Value.HitboxOffset.x - context.attackBoxFlexibilityMargin ))
                {
